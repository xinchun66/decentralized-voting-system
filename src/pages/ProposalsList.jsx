...
export default function ProposalsList() {
  ...
  return (
    <div>
      ...
      <p className="page-subtitle">查看经费申请通过率、票权分布与结果公示</p>
          ...
          {loading ? (
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {proposals.map((item) => {
                ...
                const funding = getFundingRecommendation(item.yesVotes, item.noVotes);
                return (
                  <Link key={item.id} to={`/proposal/${item.id}`} className="proposal-card">
                    ...
                    <p style={{ fontSize: 13, color: "#5b21b6", margin: "0 0 10px", textAlign: "center" }}>
                      {funding.message}
                    </p>
                    <div style={{ display: "flex", gap: 32, justifyContent: "center" }}>
                      <div style={{ textAlign: "center" }}>
                        <p style={{ color: "#16a34a", fontWeight: 700, margin: 0 }}>同意 {rates.yesRate}%</p>
                        <p style={{ fontSize: 13, color: "#64748b", margin: 4 }}>{item.yesVotes} 票权</p>
                      </div>
                      <div style={{ textAlign: "center" }}>
                        <p style={{ color: "#dc2626", fontWeight: 700, margin: 0 }}>不同意 {rates.noRate}%</p>
                        <p style={{ fontSize: 13, color: "#64748b", margin: 4 }}>{item.noVotes} 票权</p>
                      </div>
                    </div>
                  </Link>
                );
