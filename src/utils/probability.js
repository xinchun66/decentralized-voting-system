...
export function calcProbability(yes, no) {
  ...
  const total = y + n;
  if (total === 0) {
    return { yesRate: "50.00", noRate: "50.00", yesNum: 50, noNum: 50 };
  }
 ...
 * 模拟积分结算（前端演示，不上链）
 * @param {object} opts
 * @param {boolean} opts.requiresPoints
 * @param {boolean} opts.useWeight
 */
export function simulateSettlement(yes, no, userBetSide, opts = {}) {
  ...
  if (win) {
    ...
    if (requiresPoints) {
      delta = weight * 2;
      msg = `预测正确，模拟返还质押并奖励，净增约 +${weight} 积分（链上真实结算请点「链上结算」）`;
    } else {
      delta = weight;
      msg = `预测正确，模拟奖励 +${weight} 积分`;
    }
    return { result: "win", winner, delta, message: msg };
  }
  ...
  if (requiresPoints) {
    delta = -weight * 2;
    msg = `预测错误，模拟额外扣除 ${weight} 积分（投票时已扣质押 ${weight}）`;
  } else if (useWeight) {
    delta = -weight;
    msg = `预测错误，模拟扣除 ${weight} 积分（按权重罚分）`;
  } else {
    delta = -1;
    msg = `预测错误，模拟扣除 1 积分`;
  }
  return {
    ...
    winner,
    delta,
    message: `市场结果：${yesWins ? "YES" : "NO"} 胜出。${msg}`,
  };
}
