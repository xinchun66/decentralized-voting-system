...
const CONTRACT_ABI = [
  ...
  "function createProposal(string,uint256,bool)",
...
async function runTests() {
  ...
  console.log("\n=== 测试阶段2: 创建新提案 ===");
  const proposals = [
    { title: "校园活动预算投票", duration: 24, usePoints: false },
    { title: "班级委员会选举", duration: 48, usePoints: true },
    { title: "课程改革提案", duration: 72, usePoints: false }
  ];
  ...
  for (let i = 0; i < proposals.length; i++) {
    try {
      const tx = await deployerContract.createProposal(proposals[i].title, proposals[i].duration, proposals[i].usePoints);
      await tx.wait();
